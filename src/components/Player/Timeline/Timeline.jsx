...
export default class Timeline extends PureComponent {
    ...
    const workerCode = `
      ...
      const renderCanvas = () => {
        ...
        for (let y = 0; y < height; ++y) {
          for (let x = 0; x < width; ++x) {
            if (!slices[x]) return
            const value = slices[x][height - y - 1]
            data[y * width + x] =
                    (255 << 24) | // alpha
      ...
      onmessage = e => {
        if (e.data.canvas) {
          ...
          slices = new Array(e.data.canvas.width)
        }
        if (e.data.slices) {
  ...
  componentDidMount () {
    ...
    this.worker.postMessage({ canvas: offscreen, secondsPerPixel: this.state.secondsPerPixel }, [offscreen])
    this.setState({
  ...
  async renderFrequencyDataAroundCurrentTime () {
    ...
    const startPixel = Math.max(
      0,
      Math.round(this.props.currentTime / this.state.secondsPerPixel - this.state.graphWidth)
    )
    ...
    for (const index of rangeToRender) {
      const time = this.state.secondsPerPixel * index + 1
      promises.push((async () => {
  ...
  resize () {
    this.setState({ graphWidth: this.graphContainer.clientWidth })
  }
  canvasChanged (canvas) {
    this.canvas = canvas
  }
  render () {
    const offsetPixels = this.props.audioBuffer ? Math.min(
      Math.max(this.props.currentTime / this.state.secondsPerPixel - this.state.graphWidth / 2, 0),
      this.props.audioBuffer.duration / this.state.secondsPerPixel - this.state.graphWidth
    ) : 0
    return <div className='timeline'>
      <div className='frequencygraph' ref={el => { this.graphContainer = el }}>
        <canvas
          ref={(canvas) => this.canvasChanged(canvas)}
          key={this.state.secondsPerPixel + this.state.graphWidth}
